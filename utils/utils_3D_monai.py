...
def train_on_epoch(model, dataloader, optimizer, step_times,
    ...
    for i, data in enumerate(dataloader):
        ...
        # print(images.shape)
        # print(masks.shape)
        batch_images, batch_masks = data['image'].to(device, non_blocking=True), data['label'].long().to(device, non_blocking=True)
        ...
        for indice_out in range(len(dynamic_weight)):
            for loss_name, loss_value in zip(criterion_name_total[indice_out],
                ...
                writer.add_scalar(f'train/{loss_name}'+' layer'+f'{str(indice_out)}', 
                                  loss_value, global_step=(global_step + i))
...
def eval_on_epoch(model, dataloader, 
    ...
    for i, data in enumerate(dataloader):
        images, masks = data['image'].to(device), data['label'].long().to(device)
        with torch.no_grad():
